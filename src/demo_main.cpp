...
/**
 * Demo application showcasing the KVC++ store functionality
 * This creates sample data and demonstrates various operations
 */
int main() {
    try {
        std::cout << "=== KVC++ Store Demo Application ===" << std::endl;
        ...
        std::cout << "[PASS] Created " << store.size() << " sample entries" << std::endl;
        std::cout << std::endl;

        // Demo 2: Basic operations
        std::cout << "2. Demonstrating basic operations..." << std::endl;

        // Get operation
        std::cout << "Getting 'sde_bootcamp':" << std::endl;
        const auto* course = store.get("sde_bootcamp");
        if(course) {
        ...
        std::cout << "3. Demonstrating search functionality..." << std::endl;
        ...
        try {
            ...
            std::cout << "[PASS] Successfully added user_004 with consistent types" << std::endl;
            ...
            std::cout << "✗ ERROR: Should have failed due to type mismatch!" << std::endl;

        }
        catch(const kvcpp::exceptions::TypeMismatchException& e) {
            std::cout << "[PASS] Correctly caught type mismatch: " << e.what() << std::endl;
        }
        std::cout << std::endl;
        ...
        std::string demoFile = "store/demo_store.json";
        std::cout << "Saving store to: " << demoFile << std::endl;
        store.save(demoFile);
        std::cout << "[PASS] Store saved successfully" << std::endl;

        // Create new store and load
        kvcpp::core::KeyValueStore newStore;
        std::cout << "Loading into new store..." << std::endl;
        newStore.load(demoFile);
        std::cout << "[PASS] Loaded " << newStore.size() << " entries into new store" << std::endl;

        // Verify the data
        const auto* loadedCourse = newStore.get("sde_bootcamp");
        if(loadedCourse) {
            std::cout << "[PASS] Verified loaded data: " << loadedCourse->toString() << std::endl;
        }
        std::cout << std::endl;
        ...
        std::cout << "[PASS] Updated 'sde_bootcamp' entry" << std::endl;
        ...
        std::cout << (deleted ? "[PASS]" : "✗") << " Delete operation " << (deleted ? "successful" : "failed") << std::endl;
        std::cout << "Store now has " << store.size() << " entries" << std::endl;
        std::cout << std::endl;

        std::cout << "=== Demo Completed Successfully ===" << std::endl;
        std::cout << "You can now experiment with the CLI tools:" << std::endl;
        std::cout << "  - kvstore-cli (interactive mode)" << std::endl;
        std::cout << "  - kvc++ (single command mode)" << std::endl;
